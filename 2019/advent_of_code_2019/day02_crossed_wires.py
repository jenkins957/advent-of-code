...
def find_lowest_intersection(intersections):
    lowest = sys.maxsize

    for intersection in intersections:
        value = abs(intersection[0]) + abs(intersection[1])
...
def find_minimum_signal_path(path1, path2, intersects):
    ...
    for intersect in intersects:
        ...
        for coordinate in path1:
            distance_path1 += 1
            if coordinate == intersect:
                break

        distance_path2 = 0
        for coordinate in path2:
            distance_path2 += 1
            if coordinate == intersect:
                break

        total_distance = distance_path1 + distance_path2
        lowest = min(lowest, total_distance)
...
class TestCrossedWires(unittest.TestCase):
    def test_part1(self):
        # Part 1 - Find lowest Manhattan Distance for intersect of two paths
        path1 = get_path_coordinates(input1)
        path2 = get_path_coordinates(input2)

        intersections = set(path1).intersection(path2)
        result = find_lowest_intersection(intersections)

        print('Manhattan Distance: ', result)
        self.assertEqual(1017, result)

    def test_part2(self):
        # Part 2 - Find fewest combined steps the wires must take to reach an intersection
        path1 = get_path_coordinates(input1)
        path2 = get_path_coordinates(input2)
        intersections = set(path1).intersection(path2)

        result = find_minimum_signal_path(path1, path2, intersections)
        print('Lowest Steps for intersect:', result)
        self.assertEqual(11432, result)
